...
def test_full_overlap_single_bin():
    ...
    p = Presence(boundary=dummy_boundary, element=Entity(), start=1.0, end=2.0)
    pm = PresenceMap(p, ts)
    assert pm.is_mapped
...
def test_partial_overlap_start_and_end():
    ts = Timescale(t0=0.0, t1=5.0, bin_width=1.0)
    p = Presence(boundary=dummy_boundary, element=Entity(), start=0.25, end=2.75)
    pm = PresenceMap(p, ts)
    assert pm.is_mapped
...
def test_unmapped_presence():
    ts = Timescale(t0=0.0, t1=5.0, bin_width=1.0)
    p = Presence(boundary=dummy_boundary, element=Entity(), start=6.0, end=8.0)
    pm = PresenceMap(p, ts)
...
def test_exact_bin_edges():
    ts = Timescale(t0=0.0, t1=5.0, bin_width=1.0)
    p = Presence(boundary=dummy_boundary, element=Entity(), start=2.0, end=4.0)
    pm = PresenceMap(p, ts)
    assert pm.is_mapped
...
def test_same_bin_partial_presence():
    ts = Timescale(t0=0.0, t1=5.0, bin_width=1.0)
    p = Presence(boundary=dummy_boundary, element=Entity(), start=1.1, end=1.4)
    pm = PresenceMap(p, ts)
    assert pm.is_mapped
    assert pm.start_bin == 1
    assert pm.end_bin == 2
...
def test_full_overlap_returns_correct_value(simple_timescale):
    presence = Presence(start=2.0, end=6.0, element=Entity(), boundary=dummy_boundary)
...
def test_no_overlap_returns_zero(simple_timescale):
    presence = Presence(start=2.0, end=4.0, element=Entity(), boundary=dummy_boundary)
    pm = PresenceMap(presence, simple_timescale)
...
def test_exact_bin_match(simple_timescale):
    presence = Presence(start=3.0, end=4.0, element=Entity(), boundary=dummy_boundary)
    pm = PresenceMap(presence, simple_timescale)
...
def test_identity_of_presence_value_property(simple_timescale):
    presence = Presence(start=1.5, end=5.5, element=Entity(), boundary=dummy_boundary)
    pm = PresenceMap(presence, simple_timescale)
...
def test_multi_bin_presence_with_wide_bins():
    ts = Timescale(t0=0.0, t1=10.0, bin_width=2.0)
    presence = Presence(start=1.0, end=7.0, element=Entity(), boundary=dummy_boundary)
    pm = PresenceMap(presence, ts)
