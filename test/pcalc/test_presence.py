...
def test_overlap_true():
    p = Presence("x", MockBoundary(), 1.0, 5.0)
...
def test_duration():
    p = Presence("x", MockBoundary(), 1.0, 4.0)
    assert p.duration() == 3.0

def test_duration_inf():
    p = Presence("x", MockBoundary(), 1.0, np.inf)
...
def test_residence_time_no_overlap():
    p = Presence("x", MockBoundary(), 1.0, 2.0)
    assert p.residence_time(2.0, 3.0) == 0.0

def test_str_representation():
    p = Presence("x", MockBoundary("B"), 3.0, 7.0, "observed")
    s = str(p)
    assert "Presence(element=x, boundary=B, interval=[3.0, 7.0), provenance=observed)" in s
